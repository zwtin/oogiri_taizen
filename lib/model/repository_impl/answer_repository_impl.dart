...
class AnswerRepositoryImpl implements AnswerRepository {
  ...
  Future<AnswerModel> getAnswer({
    ...
    assert(answerId.isNotEmpty);

    return _firestore.collection('answers').doc(answerId).get().then(
      (DocumentSnapshot snapshot) {
        return AnswerModel()
          ..id = snapshot.data()['id'] as String
          ..text = snapshot.data()['text'] as String
          ..viewedTime = snapshot.data()['viewed_time'] as int
          ..likedTime = snapshot.data()['liked_time'] as int
          ..favoredTime = snapshot.data()['favored_time'] as int
          ..point = snapshot.data()['point'] as int
          ..createdAt = (snapshot.data()['created_at'] as Timestamp).toDate()
          ..topic = snapshot.data()['topic'] as String
