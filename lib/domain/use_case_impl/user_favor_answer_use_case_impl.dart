...
final userFavorAnswerUseCaseProvider = Provider.autoDispose
    .family<UserFavorAnswerUseCase, Tuple2<UniqueKey, String>>(
  (ref, tuple) {
    final userFavorAnswerUseCase = UserFavorAnswerUseCaseImpl(
      tuple.item1,
      tuple.item2,
    ...
    ref.onDispose(userFavorAnswerUseCase.disposed);
    return userFavorAnswerUseCase;
  },
);

class UserFavorAnswerUseCaseImpl implements UserFavorAnswerUseCase {
  UserFavorAnswerUseCaseImpl(
    this._key,
    this._userId,
  ...
  Future<Result<void>> fetchAnswers() async {
    ...
    if (_answers.list.isEmpty) {
      ...
      final lastAnswerId = _answers.list.last.id;
      final favoredTimeResult = await _favorRepository.getFavoredTime(
        userId: _userId,
        answerId: lastAnswerId,
      );
      if (favoredTimeResult is Failure) {
        return Result.failure(OTException(alertMessage: 'お気に入りのボケが読み込めませんでした'));
      }
      offset = (favoredTimeResult as Success<DateTime>).value;
    }
    const limit = 10;
    _isConnecting = true;
    final favoredAnswerIdsResult = await _answerRepository.getFavorAnswerIds(
      userId: _userId,
      offset: offset,
      limit: limit + 1,
    );
    if (favoredAnswerIdsResult is Failure) {
      return Result.failure(OTException(alertMessage: 'お気に入りのボケが読み込めませんでした'));
    }
    final favoredAnswerIds =
        (favoredAnswerIdsResult as Success<List<String>>).value;
    final hasNext = favoredAnswerIds.length == limit + 1;
    if (hasNext) {
      favoredAnswerIds.removeLast();
    }
    final favoredAnswers = _answers.list;
    for (final id in favoredAnswerIds) {
      final favoredAnswerResult = await _answerRepository.getAnswer(id: id);
      if (favoredAnswerResult is Failure) {
        continue;
      }
      var favoredAnswer = (favoredAnswerResult as Success<Answer>).value;
      ...
      if (_loginUserId != null) {
        ...
        if (isLikeResult is Success<bool> && isFavorResult is Success<bool>) {
          favoredAnswer = favoredAnswer.copyWith(
            isLike: isLikeResult.value,
            isFavor: isFavorResult.value,
          );
        }
      }
      favoredAnswers.add(favoredAnswer);
    ...
    _isConnecting = false;
    _setAnswers(
      answers: _answers.copyWith(list: favoredAnswers, hasNext: hasNext),
    );
    return const Result.success(null);
  }
  ...
  Future<void> disposed() async {
    ...
    debugPrint('UserFavorAnswerUseCaseImpl disposed $_key');
  }
}
